...
def train_epoch(model, loader, optimizer):
    ...
    #print('Melanoma: {}, Rest: {}'.format(mel_cnt, cnt))
    #print(trgts)
    return train_loss
...
def run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, mel_idx, diag_idx):
    ...
    if args.out_dim == 2:
        ...
        indices = df_train.index[df_train.diagnosis == 'melanoma'].tolist()

        num_samples = len(df_train)
        weights = [1 for _ in range(num_samples)]

        for i in indices: weights[i] = 10
    ...
    if args.out_dim > 2:
        ...
        train_loader = torch.utils.data.DataLoader(dataset_train, batch_size=args.batch_size, sampler=WeightedRandomSampler(weights=weights, num_samples=2*len(indices), replacement=False), num_workers=args.num_workers)
...
#     scheduler_cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.n_epochs - 1)
    ...
    print('Total epochs to run: {}'.format(args.n_epochs))
...
def main():
    ...
    df, df_test, meta_features, n_meta_features, mel_idx, diag_idx = get_df(
        args.kernel_type,
        args.out_dim,
        args.data_dir,
        args.data_folder,
    ...
    for fold in folds:
        run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, mel_idx, diag_idx)
